...
from trpg.engine.items import Consumable
...
def _make_world():
    ...
    return WorldState(characters={"kaine": kaine, "goblin_1": goblin}, scene="")
...
def test_damage_tag_reduces_hp():
    ...
    with patch("trpg.engine.combat.roll", return_value=4):
        cleaned, results = parse_and_resolve(
            "地精攻擊！[DAMAGE: 1d6+2 to kaine]", world
        )
    assert world.characters["kaine"].hp == 22 - 4   # mock returns 4; modifier ignored by mock
    assert len(results) == 1
    assert "艾里亞" in results[0]
    assert "[DAMAGE:" not in cleaned


def test_consume_heal_item_restores_hp():
    # CONSUME 現在自行結算治療效果（引擎依道具 effect_value，與戰鬥 USE_ITEM 同源），
    # 不再需要獨立的 HEAL 標籤。
    world = _make_world()
    kaine = world.characters["kaine"]
    kaine.hp = 10
    kaine.consumables.append(Consumable("治療藥水", 1, "heal", "2d4+2"))
    with patch("trpg.engine.combat.roll", return_value=5):
        cleaned, results = parse_and_resolve("[CONSUME: kaine 治療藥水]", world)
    assert kaine.hp == 15                                   # 治療擲骰=5
    assert kaine.get_consumable("治療藥水").quantity == 0    # 道具已被消耗
    assert "艾里亞" in results[0] and "恢復" in results[0]
...
def test_status_tag_adds_effect():
    ...
    world = _make_world()
    cleaned, results = parse_and_resolve("[STATUS: kaine +poisoned]", world)
    assert world.characters["kaine"].has_status("poisoned")
    assert "艾里亞" in results[0]
...
def test_status_tag_removes_effect():
    ...
    world = _make_world()
    parse_and_resolve("[STATUS: kaine +poisoned]", world)
    assert world.characters["kaine"].has_status("poisoned")
    cleaned, results = parse_and_resolve("[STATUS: kaine -poisoned]", world)
    assert not world.characters["kaine"].has_status("poisoned")
