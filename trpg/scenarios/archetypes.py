...
def make_life_cleric(name: str = "Life Cleric", level: int = 3) -> Character:
    ...
    if level >= 3:
        abilities.extend(["hold_person", "spiritual_weapon_life"])
    if level >= 5:
...
def make_war_cleric(name: str = "War Cleric", level: int = 3) -> Character:
    ...
    if level >= 3:
        abilities.extend(["hold_person", "spiritual_weapon_war"])
    if level >= 6:
...
def make_devotion_paladin(name: str = "Devotion Paladin", level: int = 3) -> Character:
    ...
    if level >= 2:
        abilities.extend(["divine_smite_dev", "shield_of_faith_dev", "bless", "cure_wounds"])
    if level >= 3:
        abilities.append("sacred_weapon_dev")
    if level >= 5:
        abilities.append("misty_step")
